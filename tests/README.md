...
|---|---|---|
| `test_roundtrip.cpp` | codec: CBOR round-trips, compression, framing, hostile frames | `ctest -R roundtrip` |
| `test_network.cpp` | the network: discovery, presence, broadcast, unicast, realms, gossip, threading, large payloads, close | `ctest -R network` |
| `test_cabi.c` | the C ABI, compiled **as C**, the path P/Invoke takes | `ctest -R cabi` |
