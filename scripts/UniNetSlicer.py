    ...
    for place in places:
        ...
        for wheel in sorted(place.glob("uninet-*.whl"), reverse=True):
            ...
            if tag not in name and "py3-none-any" not in name:
                continue
            # A wheel must at least be a real, readable wheel before it is
            # trusted with anything: a crashed download or a half-written copy
            # lying around is exactly the state that follows the worst failure
            # modes, and it must never be preferred over building.
            if not _wheel_usable(wheel):
                if on_skip:
...
def _stamp_buildinfo(source: Path, commit: str) -> None:
    ...
    BUILD_SOURCE is what turns "which commit is this" into "is this still the
    current build". uninet/_version.py reads that checkout's HEAD at import and
    compares, so Slicer's console says "up to date" or "STALE: source is now at
    ...". With only BUILD_GIT there is nothing to compare against and the
    honest answer is "unknown" -- which is exactly the state the Slicer install
    was in, and the one machine where the question is asked most.
    """
    ...
    try:
        target.write_text(
            ...
            f'BUILD_DATE = {_utc_now()!r}\n'
            f'BUILD_SOURCE = {str(source.resolve())!r}\n',
            encoding="utf-8",
        )
    except OSError:
        pass
...
def _utc_now() -> str:
    """Build time, ISO-8601, matching cmake/UniBuildStamp.cmake's format."""
...
def _wheel_usable(wheel: Path) -> bool:
    """A wheel that a later pip would actually accept: a real zip with intact
    members. A truncated download or a half-written copy must never be preferred
    over building - and must not be blamed for a failure it merely caused."""
    try:
        with zipfile.ZipFile(wheel) as zf:
            return zf.testzip() is None
    except Exception:  # noqa: BLE001 - not a zip at all, or CRC trouble
        return False


def _ensure_wheel_stamp(wheel: Path, commit: str) -> bool:
    """Make the wheel carry the commit it was built from, repairing it in place.

    The fast path writes the stamp into the source tree before pip builds, and
    pip then packages it with a correct RECORD. That path is unavailable when
    the checkout is read-only (a network share - which the whole installer is
    designed to work from), or when a hard kill left a stale stamp behind. In
    both cases an unverifiable wheel would otherwise be refused by the
    staleness check and the exact same wheel would be rebuilt on every single
    install. Instead, rewrite the wheel with the correct stamp, regenerating
    dist-info/RECORD so `pip uninstall` still removes every file.

    Returns False only when the wheel cannot be read at all: leave it to pip to
    fail honestly, and to the caller to fall through to a fresh build."""
    if not commit or _wheel_commit(wheel) == commit:
        return True
    try:
    ...
    record_path = next(
        (info.filename for info, _ in entries
         if info.filename.endswith(".dist-info/RECORD")),
        None,
    )
    stamp = "uninet/_buildinfo.py"
    kept = [(info, data) for info, data in entries
            if info.filename not in (record_path, stamp)]

    written: list[tuple[str, bytes]] = []
    buffer = wheel.with_name(wheel.name + ".new")
    try:
        with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as zout:
            for info, data in kept:
                zout.writestr(info, data)
                written.append((info.filename, data))
            stamp_info = zipfile.ZipInfo(stamp)
            stamp_info.compress_type = zipfile.ZIP_DEFLATED
            stamp_data = (
                '"""Generated by UniNetSlicer.py; do not edit."""\n'
                f"BUILD_GIT = {commit!r}\n"
            ).encode("utf-8")
            zout.writestr(stamp_info, stamp_data)
            written.append((stamp, stamp_data))
            if record_path is not None:
                record_lines = []
                for path, data in written:
                    record_lines.append(
                        f"{path},{_sha256_urlsafe(data)},{len(data)}")
...
def make_wheel(slicer: Path | str | None = None, source: Path | str | None = None,
    ...
    try:
        ...
        for stale in before - {built}:
            stale.unlink()
        # The wheel must leave here carrying the commit it was built from. The
        # normal path stamps the source tree first and pip packages that; when
        # that was impossible - a read-only checkout, or a stale stamp left by
        # a hard kill - repair the wheel in place now. Without this the very
        # wheel we just built is unverifiable and the next install would refuse
        # it and rebuild the same thing from scratch.
        if commit and _wheel_commit(built) != commit:
            if _ensure_wheel_stamp(built, commit):
                say(f"stamped {built.name} ({commit[:12]})")
            else:
                warn(f"could not stamp {built.name}; it will be rebuilt when "
        ...
        # tree. It is a generated artifact that is never committed, so any
        # copy left behind (a hard kill bypasses try/finally) would only dirty
        # the checkout and mis-stamp a later manual `pip wheel`. A read-only
        # source simply cannot be cleaned, which is fine: nothing stays where
        # nothing could have grown. (Do not put this file in .gitignore:
        # scikit-build-core excludes ignored files from the wheel it builds,
        # which would strip the stamp out of the very artifact that carries it.)
        try:
            stamp_file.unlink()
        except OSError:
            pass
...
def install(slicer: Path | str | None = None, source: Path | str | None = None,
    ...
    if current_enough:
        ...
        if desired and have_build != desired:
            why = (f"{have_build[:12]}" if have_build
                   else "no readable stamp (an old build, or one that cannot "
                        "be imported)")
            say(f"installed UniNet {have} was built from {why}; the current "
    ...
    wheel = None if source else find_wheel(info, local, on_skip=lambda w, want: say(
        f"skipping {Path(w).name}: not usable"
        + (" (unreadable or truncated)" if not _wheel_usable(w)
           else f" - built from an older source (has …{_wheel_commit(w)[:12] or 'no stamp'}, "
                f"want …{(want or '')[:12]})")))
    if wheel:
...
def install_startup_hook(slicer: Path | str | None = None) -> Path:
    ...
    # Atomic: Slicer reads this file at every start, and a crash halfway
    # through rewriting it must not destroy the user's own startup code. Write
    # next to it and move the completed file into place.
    staging = rcfile.with_name(rcfile.name + ".uninet-tmp")
    staging.write_text(text, encoding="utf-8")
    os.replace(staging, rcfile)
...
def remove_startup_hook(slicer: Path | str | None = None) -> None:
    ...
    if stripped == text:
        ...
        return
    staging = rcfile.with_name(rcfile.name + ".uninet-tmp")
    staging.write_text(stripped, encoding="utf-8")
    os.replace(staging, rcfile)
